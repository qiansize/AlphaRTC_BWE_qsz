...
class RLGymEnv:
    ...
    #     self.gym_env.reset(trace_path=self.trace_path, report_interval_ms=self.step_time,
    #                        duration_time_ms=0)
        # self.trace_path = 'traces/Serial_268629844.json'
        # self.gym_env.reset(trace_path=self.trace_path.format(self.config['trace_dir']),
        ...
        #                    report_interval_ms=self.step_time, duration_time_ms=0)
        self.packet_record.reset()
        # self.receiving_rate = np.zeros(HISTORY_LENGTH)
        # self.delay = np.zeros(HISTORY_LENGTH)
        # self.loss_ratio = np.zeros(HISTORY_LENGTH)
    ...
    def get_reward(self):
        ...
        # elif self.delay < 150:
        #     self.delay_reward = -(0.25 + (self.delay - 50) / 200.0)
        # else:
        #     self.delay_reward = -(0.75 + (self.delay - 150) / 150.0)
        # # reward = 10 * self.receiving_rate / self.average_capacity_kbps / 1000.0 + 2 * self.delay_reward - 10*self.loss_ratio
        # reward = 10*self.receiving_rate /600000.0 + 2 * self.delay_reward
        #if no base delay
        if self.delay < 150:
            self.delay_reward = -self.delay/200
        elif self.delay < 300:
    ...
    def step(self, action, last_prediction, time_to_guide):
        ...
        return self.state, reward, done, bandwidth_prediction, self.delay, self.loss_ratio,self.receiving_rate
